...
function RenderFirst(product){
    ...
    "                                            <button item-value=\""+product.Id+"\" class=\"items-more__button btn-view\">\n" +
...
function viewcustom(id){
           window.location.href = "/hfshop10/AdminEditProductServlet?Id="+id;
...
function phantrang(index){
    ...
    if(listtemp.length != 0)
    {
   	...
   	listdview = document.getElementsByClassName("btn-view");
    loadAjax(listview,viewcustom);
    show_info();
}

function loadAjax(listdview,viewcustom){
    Array.prototype.forEach.call(listdview, element=>{
