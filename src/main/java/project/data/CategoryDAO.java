...
public class CategoryDAO {
	...
	public boolean CheckNameExist(String name){
		Transaction transaction = null;
		 List<Category> catesEntityList = new ArrayList<>();
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			// start a transaction
			transaction = session.beginTransaction();
			// get an object
			 ...
			 transaction = session.beginTransaction();
			 catesEntityList= session.createQuery("FROM Category cate " +
	                    "WHERE cate.Title =: name").setParameter("name",name).getResultList();
			 transaction.commit();
